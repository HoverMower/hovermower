...
HoverMower_SafetyController::HoverMower_SafetyController(std::string name) : Node(name)
{

    // Register  publisher
    pub_eStop = create_publisher<std_msgs::msg::Bool>("/e_stop", 3);
    sub_battery = create_subscription<sensor_msgs::msg::BatteryState>("hovermower/sensors/Battery", 1000, std::bind(&HoverMower_SafetyController::batteryCallback, this, std::placeholders::_1));
    ...
    callback_handle_ = this->add_on_set_parameters_callback(
        std::bind(&HoverMower_SafetyController::parametersCallback, this, std::placeholders::_1));

    RCLCPP_INFO(get_logger(), "HoverMower_SafetyController initialized");
}

HoverMower_SafetyController::~HoverMower_SafetyController()
{
}

void HoverMower_SafetyController::run()
{
...
void HoverMower_SafetyController::batteryCallback(const sensor_msgs::msg::BatteryState::SharedPtr msg)
{
}

void HoverMower_SafetyController::bumper_unstuck()
{
    ...
    if (unstuck_bumper_) // only if we need to unstuck
    {
        ...
        if (last_bumper_left_ == true || last_bumper_right_ == true)
        {
            ...
            {
                RCLCPP_INFO(get_logger(), "HoverMower SafetyController, reverse to unstuck bumper");
            ...
            {
                RCLCPP_ERROR(get_logger(), "HoverMower SafetyController unable to unstuck bumper");
        ...
        else
        {
            ...
            RCLCPP_INFO(get_logger(), "HoverMower SafetyController unstuck bumper complete");
        }
    }
}
void HoverMower_SafetyController::perimeter_unstuck()
{
    geometry_msgs::msg::Twist cmd_vel_msg;
    rclcpp::Rate rate(50);
    ...
    if (unstuck_perimeter_) // only if we need to unstuck
    {
        ...
        if (last_peri_out_left_ == true || last_peri_out_right_ == true)
        {
            ...
            {
                RCLCPP_INFO(get_logger(), "HoverMower SafetyController, reverse to recover perimeter");
            ...
            {
                RCLCPP_ERROR(get_logger(), "HoverMower SafetyController unable to recover perimeter");
                cmd_vel_msg.linear.x = 0.0;
                cmd_vel_msg.angular.z = 0.0;
                cmd_vel_bumper->publish(cmd_vel_msg);
        ...
        else
        {
            ...
            RCLCPP_INFO(get_logger(), "HoverMower SafetyController recover perimeter complete");
        }
    }
}
void HoverMower_SafetyController::bumperleftCallback(const  std_msgs::msg::Bool::SharedPtr msg)
{
    ...
    if (unstuck_bumper_ == false && (msg->data == true ))
    {
        ...
        RCLCPP_WARN(get_logger(), "HoverMower SafetyController start unstuck bumper");
    }
    last_bumper_left_ = msg->data;
}

void HoverMower_SafetyController::bumperrightCallback(const std_msgs::msg::Bool::SharedPtr msg)
{
...
void HoverMower_SafetyController::perimeterCallback(const rosmower_msgs::msg::Perimeter::SharedPtr msg)
{
    if (unstuck_perimeter_ == false && (msg->left_inside == false || msg->right_inside == false))
    {
        ...
        RCLCPP_WARN(get_logger(), "HoverMower SafetyController start recover perimeter");
    }
    last_peri_out_left_ = !msg->left_inside;
    last_peri_out_right_ = !msg->right_inside;
}

rcl_interfaces::msg::SetParametersResult HoverMower_SafetyController::parametersCallback(
    const std::vector<rclcpp::Parameter> &parameters)
{
    ...
    RCLCPP_INFO(get_logger(), "HoverMower SafetyController Reconfigure Request: bumper_attempts: %i perimeter_attempts: %i suppress cmd_vel: %i sec bumper_reverse_time: %i ms",
                unstuck_bumper_attempts_,
                unstuck_perimeter_attempts_,
